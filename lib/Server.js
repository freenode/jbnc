...
class Server {
        ...
        return this.doServer(tlsOptions, (socket) => {
            ...
            socket.on('data', (chunk) => {
                ...
                if (true) {
                    ...
                    if (socket.connected && !socket.badauth && (global.connections[socket.hash] && global.connections[socket.hash].authenticated) && socket._outbuffer.length > 0) {
                                ...
                                case 'NICK':
                                    if (!socket.irc.password) {
                                        ...
                                        if (socket.irc.user) {
                                            ...
                                            if (global.connections[socket.hash]) {
                            ...
                            switch (command[0].toUpperCase().trim()) {
                                case 'PONG':
                                    if (command[1]) {
                                        ...
                                        else if (socket.hash && global.connections[socket.hash]) {
                                            global.connections[socket.hash].write("PONG " + command[1] + "\n");
                                        }
                                    }
                                    break;
                                case 'QUIT':
                                    ...
                                    break;
                                case 'MONITOR':
                                    if (socket.hash && global.connections[socket.hash]) {
                                        if (command[1] == "+") {
                                            if (!global.connections[socket.hash].ircv3Monitor)
                                                global.connections[socket.hash].ircv3Monitor = true;
                                            global.connections[socket.hash].write("MONITOR + " + command.slice(2).toString() + "\n");
                                        }
                                        else if (command[1] == "-") {
                                            global.connections[socket.hash].write("MONITOR - " + command.slice(2).toString() + "\n");
                                        }
                                        else {
                                            global.connections[socket.hash].write("MONITOR " + command.slice(1).toString() + "\n");
                                        }
                                    }
                                    break;
                                case 'CAP':
                                    ...
                                    continue;
                                case 'NICK':
                                    if (socket.hash && global.connections[socket.hash] && command[1]) {
                                        global.connections[socket.hash].write("NICK " + command[1] + "\n");
                                        global.connections[socket.hash].nick = command[1];
                                    }
                                    break;
                                case 'JBNC':
                                    if (!command[1]) {
                                        ...
                                        switch (command[1].toUpperCase().trim()) {
                                            case 'CHANNELS':
                                                for (let key in global.connections[socket.hash].channels) {
                                                    if (global.connections[socket.hash].channels.hasOwnProperty(key)) {
                                                        socket.write(":*jbnc NOTICE * :Active channel: " + global.connections[socket.hash].channels[key].name + "\n");
                                                    }
                                                }
                                            ...
                                            case 'USERHOSTS':
                                                for (let key in global.connections[socket.hash].channels) {
                                                    if (global.connections[socket.hash].channels.hasOwnProperty(key)) {
                                                        for (let x = 0; x < global.connections[socket.hash].channels[key].userhosts.length; x++)
                                                            socket.write(":*jbnc NOTICE * :" + x + ") " + global.connections[socket.hash].channels[key].userhosts[x] + " (" + global.connections[socket.hash].channels[key].names[x] + ")\n");
                                                    }
                                                }
                                            ...
                                            case 'OPMODE':
                                                if (command[2]) {
                                                    if (command[2].toLowerCase().trim() == "on") {
                                                        global.connections[socket.hash].opmode = true;
                                                    }
                                                    else if (command[2].toLowerCase().trim() == "off") {
                                                        global.connections[socket.hash].opmode = false;
                                                    }
                                                    else
                                                        socket.write(":*jbnc NOTICE * :Valid options are ON|OFF\n");
                                                }
                                                socket.write(":*jbnc NOTICE * :OPMODE is currently " + (global.connections[socket.hash].opmode ? "ON" : "OFF") + "\n");
                                                break;
                                            case 'STATS':
                                                if (socket.admin) {
                                                    socket.write(":*jbnc NOTICE * :" + Object.keys(global.connections).length + " IRC Connections\n");
                                            ...
                                            case 'KILL':
                                                if (socket.admin) {
                                                    if (command[2]) {
                                                        if (global.connections[command[2]]) {
                                                            key = command[2];
                                                            socket.write(":*jbnc NOTICE * :" + global.connections[key].nick_original + "!" + global.connections[key].user + "@" + global.connections[key].server + " has been disconnected\n");
                                                            global.connections[key].end();
                                                        }
                                                        else {
                                            ...
                                            case 'WHO':
                                                if (socket.admin) {
                                                    socket.write(":*jbnc NOTICE * :Listing " + Object.keys(global.connections).length + " users...\n");
                                                    for (let key in global.connections) {
                                                        if (global.connections.hasOwnProperty(key)) {
                                                            socket.write(":*jbnc NOTICE * :" + global.connections[key].nick_original + "!" + global.connections[key].user + "@" + global.connections[key].server + " (" + key + ")\n");
                                                        }
                                                    }
                                                }
                                                else {
                                                ...
                                                break;
                                            case 'CONN':
                                                socket.write(":*jbnc NOTICE * :There are " + global.connections[socket.hash].parents.length + " connections to your bouncer user connection.\n");
                                                for (let x = 0; x < global.connections[socket.hash].parents.length; x++) {
                                                    socket.write(":*jbnc NOTICE * :" + global.connections[socket.hash].parents[x].clientbuffer + " (" + global.connections[socket.hash].parents[x].remoteAddress + ")\n");
                                                }
                                                break;
                                            case 'LOGCLEAR':
                                                if (global.connections[socket.hash] && global.connections[socket.hash].buffers) {
                                                    for (let key in global.connections[socket.hash].buffers) {
                                                        if (global.connections[socket.hash].buffers.hasOwnProperty(key)) {
                                                            if (global.connections[socket.hash].buffers[key] && global.connections[socket.hash].buffers[key].privmsgnotice && global.connections[socket.hash].buffers[key].privmsgnotice.length > 0) {
                                                                global.connections[socket.hash].buffers[key].privmsgnotice.length = 0;
                                                            }
                                                        }
                                                    }
                                                }
                                                ...
                                                if (command[2]) {
                                                    if (global.connections[socket.hash] && global.connections[socket.hash].buffers) {
                                                        for (let key in global.connections[socket.hash].buffers) {
                                                            if (global.connections[socket.hash].buffers.hasOwnProperty(key)) {
                                                                if (global.connections[socket.hash].buffers[key] && global.connections[socket.hash].buffers[key].privmsgnotice && global.connections[socket.hash].buffers[key].privmsgnotice.length > 0) {
                                                                    for (let x = global.connections[socket.hash].buffers[key].privmsgnotice.length - 1; x >= 0; x--) {
                                                                        if (global.connections[socket.hash].buffers[key].privmsgnotice[x].source === command[2] && global.connections[socket.hash].buffers[key].privmsgnotice[x].target === global.connections[socket.hash].nick) {
                                                                            global.connections[socket.hash].buffers[key].privmsgnotice.splice(x, 1);
                                                                        }
                                                                    }
                                                                }
                                                            }
                                            ...
                                            case 'BUFFERS':
                                                ...
                                                for (let key in global.connections[socket.hash].buffers) {
                                                    if (global.connections[socket.hash].buffers.hasOwnProperty(key)) {
                                                        connected = global.connections[socket.hash].buffers[key].connected;
                                                        socket.write(":*jbnc NOTICE * :" + key + " (" + (connected ? "connected" : "not connected") + ") [" + global.connections[socket.hash].buffers[key].data.length + " bytes]\n");
                                                        totalbuffers++;
                                                    }
                                                }
                                            ...
                                            case 'WHOIS':
                                                if (socket.admin) {
                                                    if (command[2]) {
                                                        if (global.connections[command[2]]) {
                                                            key = command[2];
                                                            socket.write(":*jbnc NOTICE * :" + global.connections[key].nick_original + "!" + global.connections[key].user + "@" + global.connections[key].server + "\n");
                                                            socket.write(":*jbnc NOTICE * :Currently connected with " + global.connections[key].parents.length + " devices\n");
                                                            socket.write(":*jbnc NOTICE * :User is in " + Object.keys(global.connections[key].channels).length + " channels\n");
                                                        }
                                                        else {
                                                            socket.write(":*jbnc NOTICE * :No connection found by that hash.\n");
                                                        }
                                            ...
                                            case 'QUIT':
                                                ...
                                                if (typeof global.connections[socket.hash] !== 'undefined') {
                                                    global.connections[socket.hash].write("QUIT :jbnc gateway\n");
                                                    global.connections[socket.hash].end();
                                                    delete global.connections[socket.hash];
                                                }
                                                break;
                                            case 'PASS':
                                                if (command[3]) {
                                                    if (hash(command[2]) == global.connections[socket.hash].password) {
                                                        global.connections[socket.hash].password = hash(command[3]);
                                                        socket.irc.password = global.connections[socket.hash].password;
                                                        ...
                                                        global.connections[_newhash] = global.connections[socket.hash];
                                                        delete global.connections[socket.hash];
                                ...
                                default:
                                    ...
                                    if (input[i] && global.connections[socket.hash] && input[i].toString().substr(0, 4) == "JOIN") {
                                        ...
                                        for (let m = 0; m < channels.length; m++) {
                                            if (typeof global.connections[socket.hash].channels !== 'undefined' && global.connections[socket.hash].channels[channels[m].trim().toLowerCase()]) {
                                                ...
                                                if (command[2] && l < passwords.length) {
                                                    global.connections[socket.hash].write("JOIN " + channels[m].trim() + " " + passwords[l] + "\n");
                                                    l++;
                                                }
                                                else
                                                    global.connections[socket.hash].write("JOIN " + channels[m].trim() + "\n");
                                            }
                                        }
                                        break;
                                    }
                                    if (input[i] && global.connections[socket.hash] && global.connections[socket.hash].authenticated) {
                                        global.connections[socket.hash].write(input[i].toString() + "\n");
                                        for (let m = 0; m < global.connections[socket.hash].parents.length; m++) {
                                            if (global.connections[socket.hash].parents[m] == socket)
                                                continue;
                                            else if (input[i].toString().split(" ")[0] != "PONG" && input[i].toString().split(" ")[0] != "MODE" && input[i].toString().split(" ")[0] != "JOIN") {
                                                global.connections[socket.hash].parents[m].write(":" + global.connections[socket.hash].nick + " " + input[i].toString() + "\n");
                                            }
                                        }
                                        if (input[i].toString().substr(0, 7) == "PRIVMSG" || input[i].toString().substr(0, 6) == "NOTICE" || input[i].toString().substr(0, 7) == "WALLOPS" || input[i].toString().substr(0, 7) == "GLOBOPS") {
                                            for (let key in global.connections[socket.hash].buffers) {
                                                if (global.connections[socket.hash].buffers.hasOwnProperty(key)) {
                                                    if (!global.connections[socket.hash].buffers[key].connected) {
                                                        global.connections[socket.hash].buffers[key].data += ":" + global.connections[socket.hash].nick + "!" + global.connections[socket.hash].ircuser + "@" + global.connections[socket.hash].host + " " + input[i] + "\n";
                                                        if (global.connections[socket.hash].buffers[key].data.length >= global.BUFFER_MAXSIZE && global.BUFFER_MAXSIZE != 0)
                                                            delete global.connections[socket.hash].buffers[key];
                                                    }
                                                }
                                            }
                                            let count = 0;
            ...
            socket.on('close', () => {
                clearInterval(socket.pings);
                if (global.connections[socket.hash] && global.connections[socket.hash].buffers[socket.clientbuffer]) {
                    global.connections[socket.hash].buffers[socket.clientbuffer].connected = false;
                }
                if (global.connections[socket.hash]) {
                    let i=0;
                    for (let i = 0; i < global.connections[socket.hash].parents.length; i++) {
                        if (global.connections[socket.hash].parents[i] == socket)
                            break;
                    }
                    if (i < global.connections[socket.hash].parents.length) {
                        global.connections[socket.hash].parents.splice(i, 1);
                        if (global.connections[socket.hash].parents.length == 0) {
                            global.connections[socket.hash].connected = false;
                            global.connections[socket.hash].write(`AWAY :away-${Math.floor(Date.now() / 1000)}\n`);
                            if (global.BOUNCER_TIMEOUT != 0 && global.BOUNCER_TIMEOUT != null) {
                                global.connections[socket.hash].gone = setTimeout(() => { try { global.connections[socket.hash].write("QUIT :jbnc gateway\n"); global.connections[socket.hash].end(); } catch (e) { } delete global.connections[socket.hash]; }, global.BOUNCER_TIMEOUT * 1000, socket.hash);
                            }
                        }
                    }
                }
            ...
            socket.on('end', () => {
                ...
                if (global.connections[socket.hash] && global.connections[socket.hash].quitBefore005 == null) {
                    global.connections[socket.hash].quitBefore005 = true;
                }
            });
        });
    }
    ...
    clientReconnect(socket) {
        ...
        return new ClientReconnect(socket);
    }
