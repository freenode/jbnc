...
class Server {
        ...
        return this.doServer(tlsOptions, (socket) => {
            ...
            socket.pings = setInterval(function () {
                if (socket.lastping.length > 0) {
                    if (global.DEBUG) {
                        console.log(`Ending socket: lastping len > 0`);
                    }
                    socket.end();
                }
                ...
                if (socket.writable)
                    ...
                    if (global.DEBUG) {
                        console.log(`Ending socket: ClearInterval Socket Pings`);
                    }
                    socket.end();
                }
                if (global.DEBUG) {
                    ...
                    for (let i = 0; i < input.length; i++) {
                        ...
                        if (!socket.connected && !socket.badauth) {
                                ...
                                case 'PASS':
                                    if (commands[1]) {
                                        if (global.BOUNCER_PASSWORD.length > 0 && commands[1].split("||")[0] != global.BOUNCER_PASSWORD) {
                                            ...
                                            socket.badauth = true;
                                            if (global.DEBUG) {
                                                console.log(`Ending socket: Bad Auth. Incorrect Password.`)
                                            }
                                            socket.end();
                                        }
                                        else {
                                            ...
                                            if (/(.*)\|\|(.*)\/(.*)\|\|(.*)\/(.*)\/(.*)/g.test(passIrc)) {
                                                ...
                                                if (socket.irc.password.replace(/\s/g, '#').length < 6) {
                                                    ...
                                                    socket.badauth = true;
                                                    if (global.DEBUG) {
                                                        console.log(`Ending socket: Bad Auth. Min length password.`);
                                                    }
                                                    socket.end();
                                                }
                                                ...
                                                if (global.BOUNCER_MODE === "gateway") {
                                                    if (serverInfo) {
                                                        ...
                                                        if (global.DEBUG) {
                                                           console.log(`Ending socket: Random socket close for ||`);
                                                        }
                                                        socket.end();
                                                    }
                                                } else {
                                                    if (serverInfo || serverPassword || buffer) {
                                                        ...
                                                        if (global.DEBUG) {
                                                            console.log(`Ending socket: Random socket close no server password or buffer info`);
                                                        }
                                                        socket.end();
                                                    }
                                                }
                                                ...
                                                if (socket.irc.password.replace(/\s/g, '#').length < 6) {
                                                    ...
                                                    socket.badauth = true;
                                                    if (global.DEBUG) {
                                                        console.log(`Ending socket: Pass too short min len 6`);
                                                    }
                                                    socket.end();
                                                }
                                                ...
                                                if (global.BOUNCER_MODE === "gateway") {
                                                    if (serverInfo) {
                                                        ...
                                                        console.log(`Ending socket: Random || close`);
                                                        socket.end();
                                                    }
                                                } else {
                                                    if (serverInfo || serverPassword || buffer) {
                                                ...
                                                // Old system
                                                let origin = commands[1].trim().split("/");
                                                ...
                                                if (socket.irc.password.length < 6) {
                                                    socket.write(":*jbnc NOTICE * :*** Password too short (min length 6) ***\n");
                                                    socket.badauth = true;
                                                    console.log(`Ending socket: Bad Auth`)
                                                    socket.end();
                                                }
                                                // hash password
                                                socket.irc.password = hash(socket.irc.password);
                                                if (global.BOUNCER_MODE == "gateway") {
                                                    if (origin.length != 1 && origin.length != 2) {
                                                        console.log(`Ending socket: Origin Length not eq 1 or 2`);
                                                        socket.end();
                                                    }
                                                    else {
                                                        if (origin[1])
                                                    ...
                                                    if (origin.length != 2 && origin.length != 3 && origin.length != 4) {
                                                        console.log(`Ending socket: origin len`);
                                                        socket.end();
                                                    }
                                                    else {
                                        ...
                                        socket.badauth = true;
                                        console.log(`Ending socket: Bad Auth. Password too short.`)
                                        socket.end();
                                    }
                                    break;
                                case 'NICK':
                                    if (!socket.irc.password) {
                                        socket.write(":*jbnc NOTICE * :*** This is a JBNC Server.  You must set a password.\n");
                                        socket.badauth = true;
                                        console.log(`Ending socket: Client must set password.`);
                                        socket.end();
                                    }
                                    else if (commands[1]) {
                                ...
                                case 'USER':
                                    if (!socket.irc.password) {
                                        ...
                                        if (global.BOUNCER_USER.length > 0 && socket.irc.user != global.BOUNCER_USER) {
                                            ...
                                            console.log(`Ending socket: Incorrect username.`);
                                            socket.end();
                                        }
                                        else {
                                            if (socket.irc.nick) {
                                        ...
                                        socket.badauth = true;
                                        if (global.DEBUG) {
                                            console.log(`Ending socket: IRC client is faulty.`);
                                        }
                                        socket.end();
                                    }
                                    break;
                            ...
                            switch (command[0].toUpperCase().trim()) {
                                ...
                                case 'QUIT':
                                    if (global.DEBUG) {
                                        console.log(`Ending socket: Client sent quit.`);
                                    }
                                    socket.end();
                                    break;
                                case 'MONITOR':
                            ...
                            if (global.DEBUG) {
                                console.log(`Ending socket: Random undetailed socket end.`);
                            }
                            socket.end();
                        }
                    }
    ...
    listen() {
        ...
        // Web admin panel
        if (global.WEBADMINPANEL) {
            const webPanel = new WebAdminPanel(global.WEBADMINPANEL_PORT, this.connections, this.instanceConnections, doreport.getReports());
            webPanel.start();
        }
    }
